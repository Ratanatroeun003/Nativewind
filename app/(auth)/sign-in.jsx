...
export default function Page() {
        ...
        return (
            <SafeAreaView className="bg-background flex-1">
                <View className="px-6 py-6 justify-center">
                    ...
                    <Text className="text-foreground mb-2">Verification Code</Text>
                    <TextInput
                    ...
                    />
                    {errors.fields.code && <Text className="text-error text-sm mb-2">{errors.fields.code.message}</Text>}
                    <Pressable
    ...
    return (
        <SafeAreaView className='bg-background flex-1'>
            <View className='px-6 py-6 justify-center'>
                <Text className="text-foreground text-3xl font-sans-bold mb-6">Sign In</Text>

                <Text className="text-foreground text-sm mb-2 font-sans-medium">Email address</Text>
                <TextInput
                    className='bg-card border border-border text-foreground px-4 py-4 mb-4 rounded-2xl font-sans-medium focus:border-primary'
                    autoCapitalize="none"
                    value={emailAddress}
                ...
                />
                {errors.fields.identifier && <Text className="text-error text-sm mb-3">{errors.fields.identifier.message}</Text>}

                <Text className="text-foreground text-sm mb-2 font-sans-medium">Password</Text>
                <TextInput
                    className='bg-card border border-border text-foreground px-4 py-4 mb-4 rounded-2xl font-sans-medium focus:border-primary'
                    value={password}
                ...
                />
                {errors.fields.password && <Text className="text-error text-sm mb-4">{errors.fields.password.message}</Text>}
                <Pressable
                    onPress={handleSubmit}
                    disabled={isSignInDisabled}
                    className={`bg-primary py-4 rounded-2xl items-center justify-center mb-4 shadow-md shadow-primary/20 ${isSignInDisabled ? 'opacity-50' : 'active:opacity-80'}`}
                >
